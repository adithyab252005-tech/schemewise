...
/**
 * Mirrors web DetailsFillingPage.jsx (onboarding/details).
 * Multi-field form: name, age, gender, state, caste, occupation, income, BPL/disabled/student toggles.
 */
@OptIn(ExperimentalMaterial3Api::class, ExperimentalLayoutApi::class)
@Composable
fun DetailsFillingScreen(
    onSubmit: () -> Unit,
    viewModel: DetailsFillingViewModel = hiltViewModel()
) {
    ...
    var categoryConsent by remember { mutableStateOf(false) }
    ...
    Scaffold(
        ...
        bottomBar = {
            Surface(shadowElevation = 8.dp) {
                Button(
                    ...
                    else Text("Check Eligibility →", fontWeight = FontWeight.Bold, fontSize = 15.sp)
                }
        ...
        Column(
            ...
            Text("Tell us about yourself", fontSize = 22.sp, fontWeight = FontWeight.Bold, color = Brand800)
            Text("We use this to match you to eligible schemes only.", color = Muted, fontSize = 13.sp)
            ...
            Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically, modifier = Modifier.fillMaxWidth().padding(top = 8.dp)) {
                ...
                Text(
                    text = "I explicitly consent to the collection and processing of my social category data solely to assess my eligibility for government schemes. I understand providing this data is voluntary.",
                    fontSize = 11.sp,
                    color = Muted,
                    lineHeight = 14.sp
                )
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(value = district, onValueChange = { district = it }, label = { Text("District (e.g. Pune)") }, modifier = Modifier.weight(1f), singleLine = true)
                OutlinedTextField(value = city, onValueChange = { city = it }, label = { Text("City (e.g. Kothrud)") }, modifier = Modifier.weight(1f), singleLine = true)
            }
            ...
            OutlinedTextField(
                ...
                label = { Text("e.g. Farmer, Student, or specify") },
            ...
            SectionLabel("TARGETED SEARCH (OPTIONAL)")
            OutlinedTextField(
                value = interestedScheme,
                onValueChange = { interestedScheme = it },
                label = { Text("Interested in a specific scheme?") },
                placeholder = { Text("e.g. PM-KISAN") },
                modifier = Modifier.fillMaxWidth(),
                singleLine = true
            )
            ...
            Column {
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Text("Below Poverty Line (BPL)?", fontSize = 14.sp, color = OnSurface)
                    Switch(checked = isBpl, onCheckedChange = { isBpl = it })
                }
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Text("Person with Disability?", fontSize = 14.sp, color = OnSurface)
                    Switch(checked = isDisabled, onCheckedChange = { isDisabled = it })
                }
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Text("Currently a Student?", fontSize = 14.sp, color = OnSurface)
                    Switch(checked = isStudent, onCheckedChange = { isStudent = it })
                }
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Text("Are you a Farmer?", fontSize = 14.sp, color = OnSurface)
